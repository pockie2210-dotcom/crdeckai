window.META_QUALITY = META_QUALITY; 
          ...
          const html = `
            <div id="synergyModal" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.9); z-index:9999; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(4px);">
                <div style="position:relative; width:360px; height:480px; background:linear-gradient(145deg, #1a1a1a, #2a2a2a); border-radius:20px; box-shadow:0 0 40px rgba(0,210,255,0.2); border:1px solid #333; display:flex; flex-direction:column; align-items:center;">
                    ...
                    <div style="position:absolute; top:10px; right:15px; cursor:pointer; color:#666; font-size:20px;" onclick="document.getElementById('synergyModal').remove()">✕</div>
                </div>
            </div>
        `;
  ...
  <script type="module">
      ...
      // Weakness Button Logic - Disabled/Legacy Code Removed
  ...
  <script>
    (function () {
      ...
      // --- LOUD DEBUGGING & GLOBAL HANDLERS ---
      window.onerror = function(msg, url, line, col, error) {
         // Ignore minor irrelevant errors
         if (msg.includes("ResizeObserver")) return;
         alert("Create Error: " + msg + "\nLine: " + line);
         return false;
      };
      ...
      window.scanPlayerFromInput = async function() {
          ...
          if (!input || !btn) {
              alert("Critical: Input or Button element missing from DOM");
              return;
          }

          let tag = input.value.trim();
          if (!tag) {
              alert("Please enter a player tag (e.g. #2P090J0V)");
              return;
          }
           if (!tag.startsWith('#')) {
              tag = '#' + tag.toUpperCase();
          ...
          localStorage.setItem('cachedPlayerTag', tag);
      ...
      async function fetchPlayer(tag) {
          try {
              ...
              const res = await fetch(`/api/player/${safeTag}`);
              ...
              if(!data.currentDeck || data.currentDeck.length === 0) {
                  alert("Player found, but no current deck!");
                  return;
              }

              // Update Global
              window.currentAppDeck = data.currentDeck;

              // Render
              renderDeck(data);
              
              // Helper: Update Stats
              if(window.updateDeckStats) window.updateDeckStats(data.currentDeck);

              // Update UI
              const output = document.getElementById('output');
              if(output) output.innerHTML = `Loaded <b>${data.name}</b> (${data.clan ? data.clan.name : 'No Clan'})`;

          } catch (err) {
              console.error(err);
              alert("Failed to load player: " + err.message);
              const output = document.getElementById('output');
              if(output) output.innerHTML = `<span style="color:red">${err.message}</span>`;
          }
      }

      const scanInput = document.getElementById('tagInput');
      if (scanInput) {
          // Enter Key
          scanInput.addEventListener('keypress', function (e) {
              if (e.key === 'Enter') {
                  window.scanPlayerFromInput();
              }
          });
          ...
          const cachedTag = localStorage.getItem('cachedPlayerTag');
          if (cachedTag) {
              scanInput.value = cachedTag;
          }
      }



      // AI DECK DOCTOR SETUP
      const doctorBtn = document.getElementById('aiDoctorBtn');
      if (doctorBtn) {
        doctorBtn.onclick = () => {
          if (!window.currentAppDeck || window.currentAppDeck.length === 0) {
            alert("Please scan a player or build a deck first!");
            return;
          }

          // 1. Run Diagnosis
          // We pass current deck + collection (simulated for now by meta cache)
          // Ideally we pass the user's full collection if we had it, but for now we trust `META_QUALITY` or similar.
          // In simpler terms, we just need the deck to check issues.
          const diagnosis = window.SmartAI.diagnose(window.currentAppDeck);
          
          // 2. Render Score Ring
          const scoreVal = document.getElementById('doctorScoreValue');
          const scoreLabel = document.getElementById('doctorScoreLabel');
          const ring = document.getElementById('doctorScoreRing');
          
          scoreVal.innerText = diagnosis.score;
          
          let color = '#2ecc71'; // Green
          let label = "Healthy Deck";
          
          if (diagnosis.score < 60) {
            color = '#e74c3c'; // Red
            label = "Critical Condition";
          } else if (diagnosis.score < 85) {
            color = '#f1c40f'; // Yellow
          ...
          if (diagnosis.suggestions.length === 0) {
            ...
            document.getElementById('applyDoctorFixesBtn').style.display = 'none';
             ...
             diagnosis.suggestions.forEach(sugg => {
                // If it's a generic "Add X" suggestion
                const choice = sugg.choices[0]; // Take top choice
                // Find icon logic (simplified)
                const metaCard = window.allCards ? window.allCards.find(c=>c.name===choice) : { iconUrls: {medium: ''} }; 
                const iconUrl = metaCard ? metaCard.iconUrls.medium : ''; 
          ...
          console.error("AI Doctor Button not found!");
      }

      // 5. APPLY FIXES BUTTON
      const applyFixBtn = document.getElementById('applyDoctorFixesBtn');
      if (applyFixBtn) {
         applyFixBtn.onclick = () => {
             if (!window.currentAppDeck) return;
             
             // Run Fix
             const newDeck = window.SmartAI.fix(window.currentAppDeck, window.allCards || []);
             
             // Update Global State
             window.currentAppDeck = newDeck;
             if (window.currentPlayerData) {
                 window.currentPlayerData.currentDeck = newDeck;
             }
             
             // Re-Render App
             // 1. Deck Builder
             if (window.renderDeck) window.renderDeck(window.currentPlayerData || {currentDeck: newDeck});
             // 2. Refresh Doctor if still open? Or close it?
             // Let's close it and show a success toast
             ...
             // Trigger Synergy/Stats update
             if (window.updateDeckStats) window.updateDeckStats(newDeck);
         };
      }
  ...
  <script>
    (function () {
      ...
      // This logic was missing in the reverted commit but is required for buttons to wo.als
  ...
  <script type="module">
    ...
    console.log("Win Conditions Loaded:",bje(WIN_CONDITION_GUIDES).length);
